...
use vdem::{exploit::Exploit, process::TerminateProcess, utils::promote_privilege_to_debug_level};
...
fn main() {
    ...
    if let Err(e) = exploit.terminate_process(pid) {
        panic!("Terminate process {pid} failed: {e}")
    } else {
