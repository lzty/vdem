...
pub(crate) fn validate_pe_header_x86(data: *const u8) -> bool {
    let nt_hdr_ofs = (unsafe { *(data as *const IMAGE_DOS_HEADER) }).e_lfanew as u32;

    // e_lfanew is 0x108 on win10 and 0x110 on win11, but it is no guarantee that e_lfanew can be a fixed value
    if nt_hdr_ofs > 0x200 {
        return false;
    }

    // e_lfanew is not aligned to 4 bytes boundary
    if (nt_hdr_ofs & (4 - 1)) > 0 {
        return false;
    }

    let nt_header = unsafe { data.byte_offset(nt_hdr_ofs as _) } as *const IMAGE_NT_HEADERS32;
    ...
    if (unsafe { *nt_header }).FileHeader.Machine != IMAGE_FILE_MACHINE_I386 {
        return false;
    }
...
pub(crate) fn validate_pe_header_x64(data: *const u8) -> bool {
    let nt_hdr_ofs = (unsafe { *(data as *const IMAGE_DOS_HEADER) }).e_lfanew as u32;
    ...
    let nt_header = unsafe { data.byte_offset(nt_hdr_ofs as _) } as *const IMAGE_NT_HEADERS64;
    ...
    if (unsafe { *nt_header }).FileHeader.Machine != IMAGE_FILE_MACHINE_AMD64 {
        return false;
    }
